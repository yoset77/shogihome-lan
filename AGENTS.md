...
- **セキュリティ**: `resolveKifuPath` ヘルパーにより、Path Traversal 攻撃（`../../` 等）を厳格に防止しています。**また、許可された拡張子のみを操作対象とし、ディレクトリの深さ制限（最大10階層）や最大ファイル読み込み数（10,000件）を設けることで、不正なファイル操作やリソースの過剰消費を防止しています。**
